...
//		this.api_uri_scheme = api_uri_scheme;
	...
	public List<User> getUsers(int limit) {
		...
		UriComponentsBuilder uriComponentsBuilder = UriComponentsBuilder
				.fromUriString(this.api_uri_users)
...
//		String uriStr = uriComponents.toUriString();
		...
		UserData userData = this.restTemplate.getForObject(uriStr,
				UserData.class);
	...
	public Flux<User> getUsers(Mono<Integer> limit) {
		...
		return WebClient
				.create(this.api_uri_users)
				.get()
				.uri(uriBuilder -> uriBuilder.queryParam(
						"limit", limit.block()).build())
				.accept(MediaType.APPLICATION_JSON)
				.exchange()
				.flatMap(r -> r.bodyToMono(UserData.class))
